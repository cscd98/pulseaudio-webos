...
pa_cvolume_ramp_int* pa_sink_input_cvolume_ramp_convert(const pa_cvolume_ramp *src, pa_cvolume_ramp_int *dst, int sample_rate) {
    ...
    for (i = 0; i < channels; i++) {
        ...
        if (src->fade_type == PA_VOLUME_FADE_IN) {
            dst->ramps[i].start = 0.0;
            dst->ramps[i].end = 1.0;
            dst->ramps[i].target = 0x10000U;
        } else if (src->fade_type == PA_VOLUME_FADE_OUT) {
            dst->ramps[i].start = 1.0;
            dst->ramps[i].end = 0.0;
            dst->ramps[i].target = 0;
        }
    }
    j = i;
    ...
    pa_log_debug("pa_cvolume_ramp_convert length %ld left %ld fade type %d", dst->ramps[0].length, dst->ramps[0].left, src->fade_type);
    return dst;
}
