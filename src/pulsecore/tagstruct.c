...
void pa_tagstruct_put_cvolume_ramp(pa_tagstruct *t, const pa_cvolume_ramp *ramp) {
    ...
    long length;
    pa_volume_t target;
    pa_volume_fade_type_t fade_type;
    pa_assert(t);
    pa_assert(ramp);
    extend(t, 2 + ramp->channels * (sizeof(pa_volume_ramp_type_t) + sizeof(long) + sizeof(pa_volume_t)) + sizeof(pa_volume_fade_type_t));
    ...
    fade_type = htonl(ramp->fade_type);
    memcpy(t->data + t->length, &fade_type, sizeof(pa_volume_fade_type_t));
    t->length += sizeof(pa_volume_fade_type_t);
}
...
int pa_tagstruct_get_cvolume_ramp(pa_tagstruct *t, pa_cvolume_ramp *ramp) {
    ...
    if ((ramp->channels = t->data[t->rindex+1]) > PA_CHANNELS_MAX)
        return -1;
    if (t->rindex+2+ramp->channels*(sizeof(pa_volume_ramp_type_t)+sizeof(long)+sizeof(pa_volume_t)) + sizeof(pa_volume_fade_type_t) > t->length)
        return -1;
    ...
    memcpy(&fade_type, read_ptr, sizeof(pa_volume_fade_type_t));
    ramp->fade_type = (pa_volume_fade_type_t) ntohl(fade_type);
    read_ptr += sizeof(pa_volume_fade_type_t);
